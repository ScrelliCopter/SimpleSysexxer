...
void MidiOut::determineChunkAmount()
{
	...
	for (auto i : eventList)
		chunkAmount += ceil((double)i->size() / (double)CHUNKMAXSIZE);
}
...
void MidiOut::send( std::vector<unsigned char>* data )
{
	...
	for (unsigned char & i : *data)
	{
		...
		if (chunk.size() == CHUNKMAXSIZE || chunk.back() == 0xF7)
		{
			...
			{
				emit errorMessage(
					tr("Event %1 was not sent successfully.").arg(eventCount, 10),
					tr("Please send the file used to the author for analysis."));
...
void MidiOut::init()
{
	...
	portNumber = snd_seq_create_simple_port(sequencerHandle, "Out",
